...
class Player(Object):
  ...
  def __init__(self, world, pos, stage='train', cause=None, effect=None):
    super().__init__(world, pos)
    self.facing = (0, 1)
    self.inventory = {
        name: info['initial'] for name, info in constants.items.items()}

    status = ['drink', 'energy', 'food', 'health']
    materials = ['wood', 'stone', 'iron', 'coal'] # 当作材料制作物品
    tools = ['wood_pickaxe','stone_pickaxe','iron_pickaxe','wood_sword','stone_sword','iron_sword','table','furnace']
    sees = ['water','grass','stone','tree','coal','iron','table']

    # TODO 还只是简单的加上，没有区分是在sees、inventory、status中
    if stage == 'valid' and cause is not None:
      if cause in self.inventory or effect in self.inventory:
        if cause in materials and cause not in status:
          self.inventory[cause] += 5
        elif effect in status:
          self.inventory[effect] -= 2
  ...
  def update_specific(self, cause):
    target = (self.pos[0] + self.facing[0], self.pos[1] + self.facing[1])
    material, obj = self.world[target]
    materials = ['water', 'grass', 'stone', 'tree', 'coal', 'iron', 'diamond', 'furnace']
    objs = ['plant', 'fence', 'zombie', 'skeleton', 'cow']
    action = self.action
    if self.sleeping:
      if self.inventory['energy'] < constants.items['energy']['max']:
        action = 'sleep'
    ...
    if action == 'noop':
      ...
      self._move(action[len('move_'):])
    elif action == 'do' and cause in objs:
      self._do_object(cause)
    elif action == 'do' and cause in materials:
      self._do_material(target, cause)
    elif action == 'do' and obj:
      self._do_object(obj)
    elif action == 'do':
      self._do_material(target, material)
      ...
      if self.inventory['energy'] < constants.items['energy']['max']:
        self.sleeping = True
    elif action.startswith('place_') and cause in material:
      self._place(action[len('place_'):], target, cause)
    elif action.startswith('make_'):
      self._make(action[len('make_'):])
    self._update_life_stats()
    self._degen_or_regen_health()
