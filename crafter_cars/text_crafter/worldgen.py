...
def generate_world(world, player, cause=None, seed=None):
  seed = world.random.randint(0, 2 ** 31 - 1)
  ...
  for item in offsets:
    neighbour.append(player.pos + np.array(item))

  for x in range(world.area[0]):
    for y in range(world.area[1]):
      if any(np.array_equal([x, y], item) for item in neighbour) and cause is not None:
        set_specific_material(world, (x, y), player, tunnels, simplex, cause)
        if np.array_equal([x, y], player.pos + np.array([0, -1])):
          world[x, y] = 'table'
        if np.array_equal([x, y], player.pos + np.array([0, 1])):
          world[x, y] = 'furnace'
      else:
  ...
  for x in range(world.area[0]):
    for y in range(world.area[1]):
      if any(np.array_equal([x, y], item) for item in neighbour) and cause is not None:
        set_specific_object(world, (x, y), player, tunnels, cause)
      else:
        _set_object(world, (x, y), player, tunnels)

def set_specific_material(world, pos, player, tunnels, simplex, cause):
  x, y = pos
  material = ['water', 'grass', 'stone', 'path', 'sand', 'tree', 'lava', 'coal', 'iron', 'diamond', 'table', 'furnace']
  if cause in material:
    world[x, y] = cause
  elif cause == 'wood':
    world[x, y] = 'tree'
  else:
    _set_material(world, (x, y), player, tunnels, simplex)
def set_specific_object(world, pos, player, tunnels, cause):
  x, y = pos
  objs = {'cow':'grass', 'skeleton':'path'}
  if cause in objs:
    material = objs[cause]
    world[x, y] = material
    if material not in constants.walkable:
      pass
    elif material == 'grass' and cause == 'cow':
      world.add(objects.Cow(world, (x, y)))
    elif cause == 'zombie':
      world.add(objects.Zombie(world, (x, y), player))
    elif material == 'path' and cause == 'skeleton':
