...
class World:
    ...
    def __init__(self, area, materials, chunk_size, seed):
        self.area = area
    ...
    def reset(self, seed):
        ...
        self.daylight = 0.0
        self._chunks = collections.defaultdict(dict)
        self._objects = [None]
    ...
    def add(self, obj):
        ...
        self._chunks[self.chunk_key(obj.pos)][obj] = None
    ...
    def remove(self, obj):
        ...
        self._chunks[self.chunk_key(obj.pos)].pop(obj)
        obj.removed = True
    ...
    def move(self, obj, pos):
        ...
        if old_chunk != new_chunk:
            self._chunks[old_chunk].pop(obj)
            self._chunks[new_chunk][obj] = None
        obj.pos = pos
...
class LocalView:
    ...
    def __call__(self, player, unit, objs=None):
...
class EmbeddingView(SemanticView):
    """不是Crafter环境自带的，为了得到view的文本描述而加的"""

    def __init__(self, world, obj_types, grid):
        super().__init__(world, obj_types)

        self._grid = np.array(grid)
        self._offset = self._grid // 2
        self._area = np.array(self._world.area)

        self._names = {value: key for (key, value) in self._mat_ids.items()}
        self._obj_names = {value: key.__name__ for (key, value) in self._obj_ids.items()}

        self._names.update(self._obj_names)

    def local_view(self, player_pos):
        """
        Return local view of semantic array.
        """
        center = np.array(player_pos)
        canvas = self._world._mat_map.copy()
        for obj in self._world.objects:
            canvas[tuple(obj.pos)] = self._obj_ids[type(obj)]
        local_canvas = np.zeros(self._grid)
        for x in range(self._grid[0]):
            for y in range(self._grid[1]):
                pos = center + np.array([x, y]) - self._offset
                if not _inside((0, 0), pos, self._area):
                    continue
                local_canvas[x, y] = canvas[tuple(pos)]
        return local_canvas

    def local_token_view(self, player):
        """
          Return a token representation of the observations.
          [["grass", "grass", "tree"],
           ["grass, "player", "grass"],
           ...]
        """
        local_canvas = self.local_view(player.pos)
        # TODO: Right now we replace each index with the corresponding string.
        # In the future, we should place in word embedding directly.
        name_canvas = []
        for x, row in enumerate(local_canvas):
            name_canvas.append([])
            for y, idx in enumerate(row):
                name = self._names[idx]
                if name is None:
                    name = 'Null'
                name_canvas[x].append(name)

        return name_canvas

    def local_sentence_view(self, player):
        """
          Return a sentence representation of the observations.
          "You see cows, trees, and grass"
        """
        local_canvas = self.local_view(player.pos)
        sentence_obs = "Player sees: <"

        names = []
        for x, row in enumerate(local_canvas):
            for y, idx in enumerate(row):
                name = self._names[idx]  # idx表示对应物体在data_text.yaml中materials的顺序
                if name is None:  # if we are off the screen, tokens are None
                    continue
                ...
                # if name == 'table':
                #     name = 'crafting table'  # for consistency

                if name not in ['player', 'sand', 'arrow']:  # do nto include things we cannot interact with
                    names.append(name)
        # turn names into a sorted list of unique names.
        names = sorted(list(set(names)))

        # Turn the list of names into a sentence, separated by commas with 'and' before the last item
        if len(names) == 1:
            sentence_obs += names[0]
        else:
            for i, name in enumerate(names):
                if i == len(names) - 1:
                    sentence_obs += "and " + name
                else:
                    sentence_obs += name + ", "
        sentence_obs = sentence_obs + ">"
        sentence_obs_no_target = sentence_obs
        # Add in what player is targeting
        target = (player.pos[0] + player.facing[0], player.pos[1] + player.facing[1])
        material, obj = self._world[target]
        return sentence_obs, local_canvas

class LocalSemanticView:
