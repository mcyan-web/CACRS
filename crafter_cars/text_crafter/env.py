...
from crafter_cars.text_crafter import worldgen
...
class Env(BaseClass):
    ...
    def __init__(
        ...
        self._local_token_view = engine.LocalSemanticView(self.world, [objects.Player, objects.Cow, objects.Zombie,
                                                                       objects.Skeleton, objects.Arrow, objects.Plant],
    ...
    def reset(self):
        ...
        self._unlocked = set()
        worldgen.generate_world(self.world, self.player)
        return self.obs()

    def reset_for_specific(self, stage, cause, effect):
        center = (self.world.area[0] // 2, self.world.area[1] // 2)
        self._episode += 1
        self._step = 0
        self.world.reset(seed=hash((self._seed, self._episode)) % (2 ** 31 - 1))
        self._update_time()
        self.player = objects.Player(self.world, center, stage, cause, effect)
        self._last_health = self.player.health
        self.world.add(self.player)
        self._unlocked = set()
        worldgen.generate_world(self.world, self.player, cause)
        return self.obs(), self.player
        # return self.render_specific(cause), self.player

    def step(self, action):
        local_before_action = self._local_token_view.local_token_view(self.player)
        self._step += 1
        self._update_time()
        ...
        obs = self.obs()
        reward = (self.player.health - self._last_health) / 10
        self._last_health = self.player.health
        unlocked = {
        ...
        for i, row in enumerate(local_token):
            for j, item in enumerate(row):
                if item == 'Player':
                    obj_location = (i + self.player.facing[0], j + self.player.facing[1])
        info = {
            ...
            'facing_obj_after': local_token[obj_location[0]][obj_location[1]].lower(),
            'facing_obj_before': local_before_action[obj_location[0]][obj_location[1]].lower()
        }
        if not self._reward:
            reward = 0.0
    ...
    def step_specific(self, action, canvas, cause):
        ...
        info = {
            ...
            'reward': reward,
            'player_action': self.player.action,
            'local_token': self._local_token_view.local_token_view(self.player, canvas),
            'large_obs': self.render(size=(512, 512))
        }
        if not self._reward:
            reward = 0.0
    ...
    def render(self, size=None):
        ...
        item_view = self._item_view(self.player.inventory, unit)   # TODO
