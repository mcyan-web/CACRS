...
from wrapper.goal_wrapper import CrafterGoalWrapper, CrafterLMGoalWrapper
...
class Crafter(dm_env.Environment):
    ...
    def __init__(self,
        ...
        if 'CrafterTextEnv' in env_spec['name']:  # NOT baseline, so we have goals and should log
            ...
            env = CrafterGoalWrapper(env, env_spec['env_reward'], env_spec['single_task'],
                                     single_goal_hierarchical=env_spec['single_goal_hierarchical'])
        # self._env = text_crafter.text_crafter.Recorder(  # 包括了两个记录器 (State and Video)
        #     env,
        #     logdir,
        ...
        shape = (3, screen_size, screen_size)
    ...
    def reset(self, evcg_matrix, evcg_objects):
        obs, info = self._env.reset(evcg_matrix, evcg_objects)
        self._env_reward = None
    ...
    def step(self, action, evcg_matrix, evcg_objects):
        obs, reward, done, info = self._env.step(action, evcg_matrix, evcg_objects)
        obs = self.transform_obs(obs)
        # obs = self._transform_observation(obs)
        ...
        # if done:
        #     return dm_env.termination(reward, obs), info
        # return dm_env.transition(reward, obs), info
