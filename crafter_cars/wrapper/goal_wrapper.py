...
class CrafterLMGoalWrapper(CrafterGoalWrapper):
    ...
    def step(self, action, evcg_matrix, evcg_objects):
        ...
        if self.step_num % self.query_interval == 0:
            ...
            info['evcg_matrix'] = evcg_matrix
            info['evcg_objects'] = evcg_objects
            causality_graph_text = self.causal_matrix_to_text(evcg_matrix, evcg_objects)
            goals = self._make_predictions(causality_graph_text)
            goals_str = " ".join([s.lower().strip() for s in goals])
        else:
            goals_str = " ".join([s.lower().strip() for s in self.past_goals_list])
        if self.use_sbert:
            goals_str = 'Your goal is: ' + goals_str + '.'
        obs['goal'] = goals_str
        self.step_num += 1
        # TODO: 计算transition和LLM给出sub-goals之间的相似度，将相似度作为reward
